...
class Recherche_On_Article : 
    ...
    def get_key_words_nlp(self) : 
        ...
        if hasattr(self,"publisher") and (self.publisher.lower() in "".join(self.title.split()).lower() ):
            #we check if the publisher is in the title
            ...
            for i in range(len(self.title)) : 
                j = i
                s = len(self.title[i])
                while (j < len(self.title)-1) and (s < len(self.publisher)) : 
                    j += 1 
                    s += len(self.title[j])
                ...
                for k in range(i,j+1) : 
                    if self.title[k].lower() not in self.publisher.lower() : 
                        verif = 0

                if (s == len(self.publisher) ) and (verif == 1) : 
                    for k in range(i,j+1) : 
                        self.title[k] = ""
            self.title = " ".join(self.title)
